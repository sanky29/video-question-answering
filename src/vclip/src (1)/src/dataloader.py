...
class VideoData:
    ...
    def __getitem__(self, index):
        return torch.tensor(np.array(self.batches[index // self.per_batch][str(index)])).permute(0,3,1,2)/255
...
class Data:
    ...
    def __len__(self):
        return len(self.question_data)//10
    ...
    def __getitem__(self, index):
        ...
        '''
        #get questions
        questions, attn_mask, mask, labels= self.question_data[index]
...
def get_dataloader(args, tokenizer = None, mode = 'train'):
    ...
    train_data = DataLoader(train_data, batch_size = args.train.batch_size, shuffle = True, collate_fn = _collate_fn)
    val_data = DataLoader(val_data, batch_size = args.val.batch_size, shuffle = False, collate_fn = _collate_fn)
