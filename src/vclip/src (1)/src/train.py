...
class Trainer:
    ...
    def __init__(self,args):
        ...
        #get the model
        print('-'*20, 'Fetching Model', '-'*20)

        self.model = Vclip(self.args).to(self.device)#, normalization)
        
        print('-'*20, 'Fetching dataloader', '-'*20)
        #get the data
    ...
    def get_optimizer(self):
        ...
        params = [{"params": bert_param, "lr": self.lr*0.5, "weight_decay":0.00001},
            {'params': resnet_param, "lr": self.lr*0.5},
            {'params': rest}]
        return Adam(self.model.parameters(), lr = self.lr, weight_decay=0.0001)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            pbar = tqdm(self.train_data, ncols = 110, bar_format = "{l_bar}%s{bar:50}%s{r_bar}" % (Fore.BLUE, Fore.RESET))
