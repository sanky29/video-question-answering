...
class CaptionLoader(Dataset):
    ...
    def __init__(self, json_file, tokenizer, questions_per_video, transform=None, mode = 'train'):
        """
        Args:
        ...
        self.q_to_videoid = None
        if(mode == 'test'):
            self.q_to_videoid = []
            for question in enumerate(self.question_ans_list):
                self.q_to_videoid.append(question.video_id)
            self.q_to_videoid = np.array(self.q_to_videoid)

    def __len__(self):
        if(self.q_to_videoid is None):
            return len(self.video_question_mapping)
        else:
            return len(self.q_to_videoid)
    ...
    def get_test(self, idx):
        ...
        return self.q_to_videoid[idx], questions, attn_mask, mask, labels
