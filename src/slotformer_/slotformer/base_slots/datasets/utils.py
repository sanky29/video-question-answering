...
class BaseTransformsOur(object):
    """Data pre-processing steps."""
    ...
    def __call__(self, input):
        input = transforms.functional.normalize(x, mean=self.mean, std=self.std, inplace=True).float()
        input = transforms.functional.resize(x, self.resolution)
        return input
